...
export const getCurrentSongAction = (ctx, id) => {
  ...
  getSongDetail(id).then((res) => {
    ...
    console.log('getCurrentSongAction 网络请求得到当前音乐完整信息,整合后在播放页监听', song);
    const currentSong = {
      id: song.id,
      name: song.name,
...
export const setupPlayerAction = (ctx, id) => {
  ...
  if (ctx.isInnerAudioContext) {
    console.log('setupPlayerAction 设置innerAudioContextautoplay为true');
    audioContext.autoplay = true;
  } else {
    console.log('setupPlayerAction 设置backgroundAudioManager为id', id);
